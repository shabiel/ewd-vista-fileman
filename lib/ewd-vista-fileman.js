  ...
  handlers: {
    ...
    checkLaygo: function(messageObj, session, send, finished) {
      ...
      if (filePermissions) {
        ...
        let userPermissions = this.db.symbolTable.getVar('DUZ(0)').toString().split();
    ...
    helpTexts: function(messageObj, session, send, finished) {
      // Fileman supplies 2 help texts for users.
      // -- Help Prompt
      // -- Description
      this.db.symbolTable.restore(session);

      let file  = messageObj.params.file;
      let field = messageObj.params.field;

      let helpPrompt = this.db.function({
        function:'GET1^DID',
        arguments: [file, field, '', 'HELP-PROMPT']
      }).result;
      ...
      this.db.function({
        function:'GET1^DID',
        arguments: [file, field, '', 'DESCRIPTION', 'OUT']
      });

      if (this.db.symbolTable.getVar('$O(OUT(0))')) {
        for (let i = 1; i !== ''; i = this.db.symbolTable.getVar(`$O(OUT(${i}))`)) {
          description.push(this.db.symbolTable.getVar(`OUT(${i})`));
        }
      }

      finished({helpPrompt, description, file, field});
    }
